...
export const ParsedPaperSchema = z.object({
  ...
  populationDemographics: z.object({
    ...
    requiredConditions: z.array(z.string()).optional().nullable().describe('Conditions patient must HAVE at baseline'),
    ...
    excludedConditions: z.array(z.string()).optional().nullable().describe('Conditions that exclude patients'),
  }).optional().nullable(),
  studyType: z.enum(['treatment', 'prevention']).optional().nullable().describe('Treatment (requires condition) vs prevention (studies risk of condition)'),
